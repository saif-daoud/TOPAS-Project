"""Standalone CBT simulator validation protocol.

This script compares TOPA's user simulator vs Patient-Ψ (patient_psi) with:
1) Turn-aligned generation using *real therapist turns*.
...
Example:
  python -m topa.rl.validate_simulators_cbt \
    --config topa/config/config_cbt.yaml \
    --out-dir outputs/CBT/simulator_validation \
...
try:
    from .simulation.patient_psi import PatientPsiBuilder, PatientPsiCognitiveModelCache, PatientPsiSimulator
except Exception:
    PatientPsiBuilder = None
    PatientPsiCognitiveModelCache = None
    PatientPsiSimulator = None
...
RUBRIC = [
    ...
    ("local_coherence", "Local coherence (responds to therapist turn appropriately)"),
...
def _build_scoring_prompt(transcript: str, roles: RoleLabels) -> Tuple[str, str]:
    system = (
        ...
        "treat therapist turns as context."
    )
    ...
    user = (
        ...
        "in a real CBT session. Score 0–4 for each dimension with one-sentence justification per dimension.\n\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CBT simulator validation (TOPA vs Patient-Ψ).")
    parser.add_argument("--config", default="topa/config/config_cbt.yaml", help="CBT config yaml.")
    ...
    parser.add_argument("--allow-non-cbt", action="store_true", help="Override CBT-only guard.")
    args = parser.parse_args()

    cfg = Config.load(args.config)
    if not args.allow_non_cbt:
        domain_l = str(cfg.domain or "").strip().lower()
        if domain_l and domain_l != "cbt":
            raise ValueError("This script is intended for CBT only. Use --allow-non-cbt to override.")
    roles = RoleLabels(
    ...
    builder_gen = None
    if int(args.profile_builder_max_new_tokens) > 0:
        builder_gen = build_generator_from_env("USER")
        builder_gen.max_new_tokens = int(args.profile_builder_max_new_tokens)

    cog_cache = None
    if PatientPsiCognitiveModelCache is not None:
        cog_cache = PatientPsiCognitiveModelCache(cache_dir=out_dir / "cog_cache")

    results_path = out_dir / "results.jsonl"

    real_scores_cache: Dict[str, Dict[str, Any]] = {}
    wins_sample = {"topa": 0, "patient_psi": 0, "tie": 0}
    wins_segment = {"topa": 0, "patient_psi": 0, "tie": 0}
    ...
    for seg_idx, seg in enumerate(tqdm(segments, desc="Segments")):
        ...
        segment_votes[seg.segment_id] = {"topa": 0, "patient_psi": 0, "tie": 0}

        for sample_id in tqdm(range(int(args.num_samples)), desc=f"Samples {seg.segment_id}", leave=False):
            ...
            if PatientPsiBuilder is None or PatientPsiSimulator is None:
                raise RuntimeError("PatientPsi simulator is not available.")
            builder = PatientPsiBuilder(generator=builder_gen or user_gen, cache=cog_cache)
            try:
                cog = builder.build_or_load(
                    user_idx=int(seg.user_idx),
                    session_idx=int(seg.session_idx),
                    session_metadata=session_metadata or {},
                    profile_text=conv_profile,
                )
            except:
                continue
            psi_sim = PatientPsiSimulator(
                generator=user_gen,
                cognitive_model=cog,
                profile_text=conv_profile,
                session_metadata=session_metadata or {},
            ...
            (seg_dir / f"patient_psi_sample_{sample_id}.txt").write_text(psi_transcript, encoding="utf-8")
            ...
            if rng_pair.random() < 0.5:
                ...
                map_x, map_y = "topa", "patient_psi"
            else:
                sim_x, sim_y = psi_transcript, topa_transcript
                map_x, map_y = "patient_psi", "topa"
    ...
    scores_by_sim: Dict[str, Dict[str, List[float]]] = {
        ...
        "patient_psi": {k: [] for k, _ in RUBRIC},
        "real": {k: [] for k, _ in RUBRIC},
    }
    totals_by_sim: Dict[str, List[float]] = {"topa": [], "patient_psi": [], "real": []}

    for line in results_path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        obj = json.loads(line)
        for sim in ["topa", "patient_psi"]:
    ...
    for seg_id, votes in segment_votes.items():
        if votes["topa"] > votes["patient_psi"]:
            wins_segment["topa"] += 1
        elif votes["patient_psi"] > votes["topa"]:
            wins_segment["patient_psi"] += 1
        else:
            wins_segment["tie"] += 1
    ...
    for sim in ["real", "topa", "patient_psi"]:
        sim_scores = {}
        for k, _ in RUBRIC:
