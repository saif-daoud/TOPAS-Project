...
from .annotators import (
    ...
    KnowledgeGraphAnnotator,
    KnowledgeGraphRetrievalAnnotator,
)
...
class AnnotationManager:
    ...
    def __init__(self, domain: str, domain_params: dict, data_path: str, output_path: str, annotations_params: dict, api_key: str):
        ...
        self.annotators = {
            ...
            "knowledge_graph": KnowledgeGraphAnnotator,
            "knowledge_graph_retrieval": KnowledgeGraphRetrievalAnnotator,
            "kg_retrieval": KnowledgeGraphRetrievalAnnotator,
    ...
    def _resolve_component_path(path: str, name: str) -> str:
        if name in ["macro_actions", "micro_actions", "intrinsic_reward"]:
        ...
        elif name in ["knowledge_graph", "kg", "knowledge_graph_retrieval", "kg_retrieval"]:
            root = Path(path) if Path(path).is_dir() else Path(path).parent
            direct = root / "knowledge_graph.json"
            if direct.exists():
                return str(direct)
            matches = sorted(root.rglob("knowledge_graph.json"))
            if matches:
                return str(matches[0])
            raise FileNotFoundError(f"knowledge_graph.json not found under {root}")
    ...
    def _prepare_component(loaded_comp, component_name, **kwargs):
        ...
        if component_name == "macro_actions":
            ...
            for c in loaded_comp:
                ...
                comp.append(_c)
            return comp
        elif component_name == "micro_actions":
            comp = []
            for c in loaded_comp:
                _c = deepcopy(c)
                _c.pop("confidence_score")
        ...
        for component_name in self.components:
            ...
            if component_name == "micro_actions" and exp_cfg.get("enabled", False):
                ...
                build_expert_review_set(
                    ...
                    n_clusters=exp_cfg.get("n_clusters", 25)
                )
